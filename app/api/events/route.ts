  ...
  try {
    ...
    const [rows]: any = await pool.query(
      `SELECT
        ...
        ce.title,
        ce.link as url,
        DATE_FORMAT(CONVERT_TZ(ce.starttime, @@session.time_zone, '+09:00'), '%Y-%m-%dT%H:%i:%s+09:00') as start,
        DATE_FORMAT(CONVERT_TZ(ce.endtime, @@session.time_zone, '+09:00'), '%Y-%m-%dT%H:%i:%s+09:00') as end,
        ce.site,
        ce.img,
        ce.event_type,
        ce.created_by,
